...
function add_table_to_html(option) {
    ...
    for(var i = 0; i< row_number; i++){
        ...
        for(var j= 0; j< col_number; j++){
            ...
            if(content.type == "content")
                item_input =  $("<input readonly='readonly'>"+ content.content+"</input>");
            else{
                item_td.css("border-color", "#e6db55");
    ...
    item_table.appendTo(table_block);
    table_block.attr("id", "table-block");
...
function correct_range_fill_answer(student_answer, answer) {
    ...
    if (answer.charAt(0) == "[")
        first = (student_answer >= aList[0]);
    if (answer.charAt(0) == "(")
        first = (student_answer > aList[0]);
    if (answer.charAt(answer.length - 1) == "]")
        second = (student_answer <= aList[1]);
    if (answer.charAt(answer.length - 1) == ")")
        second = (student_answer < aList[1]);
    return (first && second);
}
...
function add_qa_to_html(option) {
    ...
    if(option.stu_answer == true){
        getMyAnswer({
            ...
            rt_func:function(data){
                if(data.status == "SUCCESS"){
                    option.position.find("input").val(data.answer.body.answer)
                }
            }
        });
    }
