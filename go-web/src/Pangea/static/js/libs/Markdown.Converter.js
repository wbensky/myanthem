...
(function() {
	...
	Markdown.Converter = function() {
		...
		function _RunSpanGamut(text) {
			...
			// put in place to prevent autolinking; reset now
		...
		function writeAnchorTag(wholeMatch, m1, m2, m3, m4, m5, m6, m7) {
			...
			// to prevent auto-linking withing the link. will be converted back after the auto-linker runs
            ...
            var result;
            if(url.startWith('wiki:')){
                result = '<span class="link-wiki" wiki="' + url + '">' + link_text + "</span>";
            }else{
		...
		function _DoCodeBlocks(text) {
			...
			*/
			...
			text = text.replace(/(?:\n\n|^\n?)((?:(?:[ ]{4}|\t).*\n+)+)(\n*[ ]{0,3}[^ \t\n]|(?=~0))/g, function(wholeMatch, m1, m2) {
				...
				code_language = codeblock.split('\n')[0];
				codeblock = codeblock.substring(code_language.length+1);
				codeblock = '<pre class="brush: '+code_language+'">' + codeblock + "</pre>";
		...
		function _DoCodeSpans(text) {
			...
			text = text.replace(/(^|[^\\])(`+)([^\r]*?[^`])\2(?!`)/gm, function(wholeMatch, m1, m2, m3, m4) {
				...
				c = _EncodeCode(c);
				c = c.replace(/:\/\//g, "~P");
				// to prevent auto-linking. Not necessary in code *blocks*, but in code spans. Will be converted back after the auto-linker runs.
		...
		function _DoAutoLinks(text) {
			...
			var replacer_httplink = function(wholematch, m1) {
				return "<a href=\"" + m1 + "\">" + pluginHooks.plainLinkText(m1) + "</a>";
			}
            
			text = text.replace(/<((https?|ftp):[^'">\s]+)>/gi, replacer_httplink);
